...
- **多源知识融合**
  - 医疗知识图谱：疾病–症状–药品–科室等实体关系（[Neo4j](https://neo4j.com/)）
  - 文本向量检索：病例、说明书、文档语料（[Milvus](https://milvus.io/)）
  - 大模型问答：如 [DeepSeek](https://www.deepseek.com/)，用于生成最终回答

- **模块职责清晰**
  - [`config/`](../config/)：集中配置管理（环境变量 + 默认值）→ [查看文档](../config/README.md)
...
- [技术流程文档](./architecture/technical_workflow.md) | [对话记录系统](./architecture/conversation_history_system.md) | [版本路线图](./roadmap/v2.0_roadmap.md) | [项目根目录](../)
...
1. **数据准备与知识库构建**：从原始医疗数据（medical.json、data.jsonl 等）构建 Neo4j 知识图谱和 Milvus 向量库
2. **用户查询**：用户通过前端界面或 API 提交医疗问题
3. **并行检索**：Agent Service 同时进行向量检索（Milvus）和知识图谱查询（Neo4j via Graph Service）
4. **结果融合**：将两种检索结果融合，优先使用知识图谱的结构化结果
5. **LLM 生成答案**：DeepSeek LLM 基于融合后的上下文生成最终医疗答案
...
- **`core/` – 核心能力**
  ...
  - `graph/`：Neo4j 客户端、图模式定义、NL2Cypher Prompt、查询验证等

- **`services/` – 业务服务**
  - `agent_service.py`：整合向量检索 + PDF 检索 + 知识图谱 + LLM，提供统一 `/` 问答接口
  - `graph_service.py`：负责 NL2Cypher 生成、验证和执行，对外暴露图服务 API
...
- **功能扩展建议**
  ...
  - **增强图谱**：在 `data/` 与 Neo4j 中补充实体与关系，同时更新 `core/graph/models.py` 中的模式描述。
